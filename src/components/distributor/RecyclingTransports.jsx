...
const RecyclingTransports = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [transportsResponse, itemsResponse, manufacturersResponse] =
        await Promise.all([
      ...
      const allTransports = Array.isArray(transportsResponse?.data)
        ? transportsResponse.data
        : [];
      const recyclingOnly = allTransports.filter(
        transport => transport.type === 'Recycling Pickup'
      );

      const partners = Array.isArray(manufacturersResponse?.data)
        ? manufacturersResponse.data
        : [];
      const manufacturerPartners = partners.filter(
        partner => partner.role === 'MANUFACTURER'
      );
  ...
  const handleSubmitSchedule = async e => {
    ...
    try {
      ...
      await distributorService.createRecyclingTransport({
        ...
        scheduledPickupDate: pickupTimestamp, 
        scheduledDeliveryDate: deliveryTimestamp, 
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden mb-6">
      <div className="px-6 py-4 bg-green-50 border-b border-green-200">
        <h2 className="text-xl font-semibold">
          Products Available for Recycling Pickup
        </h2>
      </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-500">
              No products are currently available for recycling pickup.
            </p>
          </div>
        ) : (
      ...
      <div className="p-6">
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-500">
              No recycling transports have been scheduled yet.
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {recyclingTransports.map(transport => (
                  <tr key={transport.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">
                        {transport.productName || 'Recycled Item'}
                      </div>
                    </td>
            ...
            >
              ...
              {manufacturers.map(manufacturer => (
                <option
                  ...
                  className={
                    manufacturer.id === selectedItem?.manufacturerId
                      ? 'font-bold'
                      : ''
                  }
                >
    ...
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="text-gray-700">
          <p className="text-lg">
            Help customers recycle their products by scheduling pickups and
            delivering to manufacturers.
          </p>
        </div>
        <button
          onClick={fetchData}
