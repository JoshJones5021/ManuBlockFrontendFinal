...
const TransportDetails = () => {
  ...
  useEffect(() => {
    ...
    if (!transportId) {
      setError(
        'Invalid transport ID. Please return to the transports list and try again.'
      );
      setLoading(false);
      return;
    }
  ...
  const fetchTransportDetails = async () => {
    try {
      ...
      const foundTransport = response.data.find(
        t => t.id === parseInt(transportId)
      );
  ...
  const fetchBlockchainDetails = async blockchainItemId => {
    try {
      ...
      const response =
        await blockchainService.getBlockchainItemDetails(blockchainItemId);
    ...
    return (
      <div className="p-6">
        <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded relative mb-6">
          Transport not found. It may have been deleted or you don't have
        ...
        <div className="px-6 py-4 bg-gray-50 border-b">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-semibold">
              Transport #{transport.trackingNumber}
            </h2>
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <div>
                <p className="text-base">
                  Scheduled: {formatDate(transport.scheduledPickupDate)}
                </p>
                {transport.actualPickupDate && (
                  <p className="text-sm text-green-600">
                    Actual: {formatDate(transport.actualPickupDate)}
                  </p>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div>
                <p className="text-base">
                  Scheduled: {formatDate(transport.scheduledDeliveryDate)}
                </p>
                {transport.actualDeliveryDate && (
                  <p className="text-sm text-green-600">
                    Actual: {formatDate(transport.actualDeliveryDate)}
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="flex space-x-4">
              ...
              {(transport.status === 'Delivered' ||
                ...
                <span className="text-gray-500">
                  No actions available - Transport completed
                </span>
              )}
            </div>
          </div>
              ...
              <div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-base">
                      {blockchainInfo.status === 0
                        ? 'Created'
                        : blockchainInfo.status === 1
                          ? 'In Transit'
                          : blockchainInfo.status === 2
                            ? 'Processing'
                            : blockchainInfo.status === 3
                              ? 'Completed'
                              : blockchainInfo.status === 4
                                ? 'Rejected'
                                : 'Unknown'}
                    </p>
                  </div>
