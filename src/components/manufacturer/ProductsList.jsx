...
const ProductsList = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [productsResponse, supplyChainResponse] = await Promise.all([
  ...
  const fetchMaterialsBySupplyChain = async supplyChainId => {
    try {
      const supplyChainResponse =
        await supplyChainService.getSupplyChainById(supplyChainId);
      ...
      const materialsData = materialsResponses.flatMap(
  ...
  const filterMaterialsByChain = (allMaterials, chainId) => {
    ...
    const chainMaterials = allMaterials.filter(material => {
      ...
      const supplierMatches = supplierUserIds.some(
        id => String(supplierId) === String(id)
      );

      console.log(
        `Material ${material.name || 'unknown'}: active=${isActive}, supplierId=${supplierId}, supplierMatches=${supplierMatches}`
      );
