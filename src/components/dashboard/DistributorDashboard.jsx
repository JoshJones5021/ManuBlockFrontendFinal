...
const DistributorDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [transportsResponse, readyMaterialsResponse, readyOrdersResponse] =
        await Promise.all([
      ...
      const inTransitCount = transportsData.filter(
        transport => transport.status === 'In Transit'
      ).length;

      const deliveredCount = transportsData.filter(
        transport => transport.status === 'Delivered'
      ).length;

      const scheduledCount = transportsData.filter(
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div className="p-6">
            <div className="mb-4">
              <p className="text-gray-600">
                Schedule pickups for materials that are ready to be transported
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div className="p-6">
            <div className="mb-4">
              <p className="text-gray-600">
                Schedule deliveries for products that are ready to be
                transported from manufacturers to customers.
              </p>
            </div>
        ...
        {transports.length === 0 ? (
          <div className="p-6 text-center">
            <p className="text-gray-500">
              No transports found. Start by scheduling a pickup or delivery.
            </p>
          </div>
        ) : (
        ...
        <div className="flex flex-wrap -mx-2">
          <div className="px-2 w-full md:w-1/3 mb-4">
            <div className="border border-blue-200 rounded-lg p-4 h-full">
              ...
              <p className="text-sm text-gray-600">
                Schedule material pickups from suppliers and product deliveries
                to customers based on requests.
              </p>
            </div>
          </div>
          <div className="px-2 w-full md:w-1/3 mb-4">
            <div className="border border-purple-200 rounded-lg p-4 h-full">
              ...
              <p className="text-sm text-gray-600">
                Record pickups and manage in-transit shipments with real-time
                status updates.
              </p>
            </div>
          </div>
          <div className="px-2 w-full md:w-1/3 mb-4">
            <div className="border border-green-200 rounded-lg p-4 h-full">
              ...
              <p className="text-sm text-gray-600">
                Record successful deliveries and maintain a complete history of
                all transports.
              </p>
            </div>
          </div>
