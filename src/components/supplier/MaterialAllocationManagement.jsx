...
const MaterialAllocationManagement = () => {
  ...
  const handleAllocate = async () => {
    ...
    try {
      ...
      setSuccess(
        `Materials for request ${selectedRequest.requestNumber} have been allocated successfully.`
      );
      ...
      setError(
        `Failed to allocate materials: ${err.message || 'Unknown error'}`
      );
    } finally {
      setAllocating(false);
    }
    ...
    <div className="p-6">
      ...
      <div className="bg-blue-50 rounded-lg p-6 border border-blue-200">
        ...
        <p className="text-blue-700 mb-4">
          Allocation is the process of reserving materials for a specific
          request and registering them on the blockchain. Once allocated, these
          materials will be trackable throughout the supply chain.
        </p>
      ...
      {showConfirmation && selectedRequest && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-lg">
            ...
            <p className="mb-4">
              You are about to allocate materials for request{' '}
              <strong>{selectedRequest.requestNumber}</strong> from
              <strong> {selectedRequest.manufacturer.username}</strong>.
            </p>

            <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200 mb-4">
              <h3 className="text-md font-medium text-yellow-800 mb-2">
                Material Items to Allocate:
              </h3>
              <ul className="list-disc pl-5 text-yellow-700">
                {selectedRequest.items
                  ...
                  .map(item => (
                    <li key={item.id}>
                      {item.material.name} - {item.approvedQuantity}{' '}
                      {item.material.unit}
                    </li>
                  ))}
              </ul>
            ...
            <p className="text-gray-600 mb-6">
              This action will register these materials on the blockchain and
